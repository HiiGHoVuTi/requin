...
Pour tout $G=(V,E)$ un graphe, on note $Delta(G) = max{ deg s : s in V }$. Soit $G=(V,E)$ un graphe, on dit que $I subset.eq V$ est un _indépendant_ si $I^2 inter E = emptyset$.
...
#problem([*Maximum Independant Set (MIS)*],[Un graphe $G=(V,E)$],$max_(I subset.eq V) {|I| : I^2inter E = emptyset}$)
...
On remarque d'abord que $W("GLOU") = S$. Soit $x in "GLOU"$, alors $|W({x}) inter "GLOU"^*| <= d$ car sinon, ça indiquerai que les $d+1$ sommets du voisinages aurai été pris, dont $x$ et un de ses voisins (donc non indépendant, ce qui est absurde).

On a donc: 
	 $ |"GLOU"^*| = |"GLOU"^* inter V| &= |"GLOU"^* inter inter.big_(x in "GLOU") W({x})|\ &<= sum_(x in "GLOU") |"GLOU"^* inter W(x)| <= d|"GLOU"| $
	 C'est bien une $d$-approximation.
	 $square$
])
   
On dit que $G'=(V',E')$ est un _sous-graphe_ de $G$ si $V' subset.eq V$ et que $E' subset.eq E inter V'^2$ \
...
Soit $G = (V,E)$ un graphe, pour $S subset.eq V$, on pose $G[S] = (S,E inter S^2)$ le sous-graphe _induit_ par $S$.
