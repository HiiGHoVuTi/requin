...
Soit $G = (V,E)$ un graphe connexe, on dit que $A subset.eq E$ est une _couverture d'arettes_ si pour tout $v in V$, on a un $e in A$ tel que $v in e$. On dit que $A$ est une couverture d'arretes _minimale_ si $A$ est une couverture d'arrete de plus petit cardinal.
...
On pose $S = (U \\ Z) union (V inter Z)$

#question(2)[
  Montrer que pour chaque arrete $(u,v) in M$ on a $|{u,v} inter S| = 1$. 
]
