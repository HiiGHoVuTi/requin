...
#question(1)[Proposer un algorithme pour tester si un mot est primitif en $O(|w|^(3/2))$.]
#correct([
  On peut se donner :
  #rect([$ 
  ...
  La solution en $O(|w|)$ est en utillisant le tableau LPS (Least prefix also suffix). Une fois le tableau LPS calculé en $O(|w|)$, on fait juste la vérification en $O(|w|)$ de si LPS$[ |w| ]$ est une bonne puissance. Voir le sujet "Autour des préfixes" pour plus d'informations.
])

#question(2)[Étant donné un automate $cal(A)$, proposer un algorithme pour déterminer si le langage reconnu par $cal(A)$ est continuable.]
#correct([
  Une fois l'automate déterministe complet crée, il suffit de vérifier si tout les états sont co-accesible. Un simple parcours depuis tout les états pour tester si on peut atteindre des états final suffit (ou un test d'un états puit).
...
#question(3)[Montrer que tout langage rationnels continuable sur $Sigma = {a,b}$ contient une infinité de mots primitifs. Quelle est la condition sur $Sigma$ pour que cela soit vrai ?]
#correct([
   Tout viens d'une chose : le fait que la continuation est bornée en taille.
...
#question(4)[$star$ Existe-t-il un langage infini rationnel continuable ne comportant que des mots primitifs ?]
#correct([
