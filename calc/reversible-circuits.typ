...
#rect[
  ...
  quantum-circuit(
    lstick($a$), targ(), rstick($a+1$)),
  quantum-circuit(
    lstick($a$), ctrl(1), rstick($a$), [\ ],
    lstick($b$), targ(), rstick($a+b$)),
  quantum-circuit(
    lstick($a$), ctrl(2), rstick($a$), [\ ],
    lstick($b$), ctrl(1), rstick($b$), [\ ],
    lstick($c$), targ(), rstick($a b + c$)),
  ))
...
#question(0)[Montrer que tout circuit réversible $cal(C)$ constitué d'une seule porte implémente une permutation. Donner leur inverse.]
#question(1)[Comment implémenter l'inverse d'un circuit quelconque ?]
#question(2)[Montrer que dans circuit sur $n$ bits, une porte de #toffoli à $n-2$ contrôles peut être implémentée à partir d'une porte de #toffoli à $n-3$ contrôles et de portes de #toffoli.]
...
#question(0)[Donner des contre-exemples pour $n in {2, 3}$.]

Soit $cal(C)$ un circuit sur $n >= 4$ bits ne comportant qu'une porte logique. Soit $S$ l'ensemble des cycles de $f_cal(C)$.

#question(2)[Montrer qu'il existe $S_1 union.sq S_2 = S$ et une bijection $S_1 <-> S_2$ préservant la longueur des cycles. En déduire un sens du théorème.]

#question(2)[Démontrer les lemmes suivants :
...
#question(1)[Montrer que pour $n >= 6$, toute permutation paire de $FF_2^n$ peut s'écrire comme produit de paires de transpositions disjointes, c'est-à-dire :
$ Pi = product^k_(i=1) (a_i b_i)med(c_i d_i) quad "et" quad forall i in [|k|], "card"{a_i,b_i,c_i,d_i} = 4 $
]

#question(2)[Soit $n >= 7$ puis $x,y,z,t in FF_2^n$ distincts deux-à-deux. Montrer qu'il existe $cal(C)$ qui implémente $Pi$ :
